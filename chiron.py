...
# ИСПРАВЛЕНО: Если передан чистый номер (Седна/Хирон), убираем DES=, оставляя только номер и %3B
if spkid.isdigit():
    command_param = "'{}%3B'".format(spkid)
else:
    command_param = "'DES%3D{}%3B'".format(spkid)

url += "&COMMAND={}&START_TIME='{}'&STOP_TIME='{}'".format(command_param, start_time, stop_time)
